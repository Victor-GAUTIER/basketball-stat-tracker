...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        # Connexions signaux
        self.video_panel.video_loaded.connect(
            self._on_video_loaded
        )

        self.player_panel.player_selected.connect(
            self.controller.set_active_player
        )

        self.player_panel.player_added.connect(
            self._on_player_added
        )
        ...
        self.event_panel.undo_requested.connect(
            self._on_undo
        )
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_Space:
            ...
            self.video_panel.play_pause()
            return


        # Ctrl + Z : annuler dernier événement
        if (
            event.key() == Qt.Key_Z
        ...
        # Les touches joueurs (1-9)
        # sont envoyées au PlayerPanel
        if Qt.Key_0 <= event.key() <= Qt.Key_9:

            self.player_panel.keyPressEvent(event)
            return
    ...
    def _on_event_triggered(self, event_type: str):
        ...
        self.event_panel.add_history_line(
            f"{timestamp:.2f}s - {event_type}"
        )
    ...
    def _refresh_stats(self):
        ...
        summary = player_summary(
            self.db,
            self.controller.current_game.id
        )
